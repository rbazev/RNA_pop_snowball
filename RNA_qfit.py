...
class Population(object):
    ...
    def __init__(self, ref_seq, pop_size, U_rate=1e-4, r_prob=0., alpha=12, w_min=0.7):
        ...
        self.w_slope = np.round(np.divide(w_min - 1, alpha), decimals=3)
    ...
    def cal_pop_fitness(self, pop, with_nl=True):
        ...
        w_viable = np.where(w_unique >= self.w_min)[0]
        ...
        if len(w_viable) == 0:
            ...
            w_redist = np.repeat(w_unique[w_viable], [len(indices) for indices in index])
            flattened_index = np.concatenate(index)
            w_pop[flattened_index] = w_redist
        return w_pop
...
class TwoPops(object):
    ...
    def diverge(self, target_D=0.1,  tot_sample=11, verbose=False):
        ...
        output_t0 = pd.DataFrame.from_dict({'gen': [0], 'w_pop1': [self.pop1.pop_fitness], 'w_pop2': [self.pop2.pop_fitness], 'dt': [0.0], 'net_size': [net_size], 'D':div['D'], 'II': [sin_II], 'segII': [seg_II], 'nu':[nu], 'HS': div['HS'], 'HT': div['HT'], 'GST':div['GST'], 'RI': RI_assay['RI'], 'RI_max': RI_assay['RI_max'], 'RI_max_free':RI_assay['RI_max_free']})
        ...
        while (curr_D < target_D) or (n_sample < tot_sample):
            ...
            if curr_target <= curr_D < curr_target + stp:
                ...
                div = {key: np.round(value, decimals=3) for key, value in div.items()}
                net_size = len(fitness_dict)
                output_t = pd.DataFrame.from_dict({'gen': [count], 'w_pop1': [self.pop1.pop_fitness], 'w_pop2': [self.pop2.pop_fitness], 'dt': [delta_t], 'net_size': [net_size], 'D':div['D'],'II': [sin_II], 'segII': [seg_II], 'nu':[nu], 'HS': div['HS'], 'HT': div['HT'], 'GST':div['GST'], 'RI': RI_assay['RI'], 'RI_max':RI_assay['RI_max'], 'RI_max_free':RI_assay['RI_max_free']})
